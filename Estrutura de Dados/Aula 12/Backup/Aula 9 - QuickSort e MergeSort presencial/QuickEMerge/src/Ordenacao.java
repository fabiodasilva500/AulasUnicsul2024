	 ...
	 int inicioB, int fimB){
		...
		*/
		public static void quickSort (int vet[], int ini, int fim)
		{
			int divisao; 
			if (ini < fim) { 
				divisao = separa(vet, ini, fim); // separa a partir do primeiro
				quickSort (vet, ini, divisao-1); // ordena a primeira parte 
				quickSort (vet, divisao+1, fim); // ordena a segunda parte
			}
		}
		
		/**
		Separa e organiza os dados
		@param vet[] vetor com os elementos desordenados
		@param ini índice ínicial
